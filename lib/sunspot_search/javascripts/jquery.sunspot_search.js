(function($){
  $.fn.sunspotSearch = function(config){
    ...
    form.data('condition-template', form.find('.condition:last').first().clone());
    form.data('choices-template', form.find('fieldset.condition:last li.choices').clone());
    form.data('value-template', form.find('fieldset.condition:last li.value').clone());

    var replaceWithContext = function(node, template) {
      var fieldset = node.closest('fieldset');
      var newIndex = node.closest('form').find('fieldset.condition').index(fieldset);
      var newHtml = template.html().replace(/_\d+_/g, '_' + newIndex + '_'); 
    ...
    $('.add_condition', form).click(function(ev){
      ...
      if(!form.data('number-conditions')) {
        form.data('number-conditions', $('.condition', form).length);
      }

      var newIndex = form.data('number-conditions') + 1;

      var newHtml = form.data('condition-template').html().replace(/_\d+_/g, '_' + newIndex + '_'); 
      newHtml = newHtml.replace(/\[\d+\]/g, '[' + newIndex + ']');

      var newCondition = form.data('condition-template').clone();
      ...
      form.data('number-conditions', newIndex + 1);
    });
    ...
    $('select.condition_attribute', form).live('change', function(ev){
      ...
      // ensure the HTML is in a blank state
      replaceWithContext(fieldset.find('li.value'), form.data('value-template'));
      replaceWithContext(fieldset.find('li.choices'), form.data('choices-template'));

      // and ensure they're hidden
      ...
      if(conditions[selected].hasOwnProperty('allow')) {
        // configuration has choosen to only allow specifc operators
        operators = conditions[selected].allow;
      } else {
        // use the default operators for this stype
        operators = attributeOperators[selectedType];
      }

      var options = $.map(operators.sort(), function(operator){
      ...
      if(conditions[selected].hasOwnProperty('choices') || conditions[selected].type === 'boolean'){
        ...
        // fire the change event manually if there is only one operator 
        // since the user cannot do it
        if(operators.length == 1) { fieldset.find('select.condition_operator').change(); }
      }

      // finally set the type and dynamic value
      fieldset.find('input.type').val(conditions[selected].type);
      if(conditions[selected].hasOwnProperty('dynamic')) {
        fieldset.find('input.dynamic').val(conditions[selected].dynamic);
      }
    });
    ...
    $('select.condition_operator', form).live('change', function(ev){
      ...
      if(selectedAttribute.hasOwnProperty('choices') || selectedAttribute.type == 'boolean'){
        ...
        if(selectedOperator == 'blank' || selectedOperator == 'not_blank') {
          ...
          // else hide it
          if(selectedAttribute.hasOwnProperty('hint')) {
          ...
          if(transforms.hasOwnProperty(selectedAttribute.type)) {
